...
class ETM(object):
    ...
    def __init__(
        ...
        num_topics=50,                  # Fewer topics for better interpretability; tune based on your data
        rho_size=300,                   # Size of topic embedding
        emb_size=300,                   # Size of word embedding (same as rho_size)
        t_hidden_size=2048,              # Reduced hidden size for better generalization and speed
        theta_act='leakyrelu',               # ReLU activation function
        train_embeddings=True,          # Fine-tune embeddings for better performance on your specific data
        lr=0.002,                       # Slightly lower learning rate for stable training
        ...
        enc_drop=0.4,                   # Dropout to prevent overfitting
        clip=0.5,                       # Gradient clipping value; set if needed to prevent exploding gradients
        nonmono=10,                     # Non-monotonicity parameter
        wdecay=1e-5,                    # Weight decay for regularization
        ...
        debug_mode=False,
        additional_hidden_size=600,
        additional_activation='selu'
    ):
    ...
    def get_topic_word_beta_matrix(self):
        """
        Obtains the topic word matrix learned for the model.
        ...
        with torch.no_grad():
            beta = self.model.get_beta().cpu().numpy()  # Get the relevance scores

            topics = []
            for i in range(self.num_topics):
                words = [self.vocabulary[a] for a in np.argsort(beta[i])[::-1]]
                topics.append(words)
